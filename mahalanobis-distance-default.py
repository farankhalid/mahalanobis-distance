...
firm_vector_filtered = firm_vector[common_columns].to_numpy()
...
for index, row in patent_vector.iterrows():
    if row.name in patent_shares["publn_nr"].values:
        ...
        if firm_year in firm_vector.index:
            firm_row = firm_vector.loc[firm_year].to_numpy()
            # print("vector 1: ", row[common_columns].shape, "vector 2: ", firm_row.shape)
...
results_df.to_csv("output/mahalanobis-results-default.csv", index=False)
print(
    "Mahalanobis distance calculation is completed and saved to output/mahalanobis-results-default.csv"
)
