...
# Chunking patents_grouped DataFrame
chunk_size = 5000
...
# Chunking patent_shares DataFrame
logging.info("Processing patent_shares DataFrame (Patent Level) in chunks.")
chunk_size_shares = 5000
patent_vector = pd.DataFrame()

for i in range(0, patent_shares.shape[0], chunk_size_shares):
    chunk = patent_shares.iloc[i : i + chunk_size_shares].copy()
    chunk["permno_year"] = chunk["permno_adj"].astype(str) + chunk["publn_year"].astype(
        str
    )
    chunk_pivoted = chunk.pivot(
        index="publn_nr", columns="IPC1", values="share_pat"
    ).fillna(0)
    patent_vector = pd.concat([patent_vector, chunk_pivoted])

# Aligning columns of firm_vector and patent_vector
...
results_df.to_csv("output/mahalanobis-results-chunking.csv", index=False)
logging.info(
    "Mahalanobis distance calculation is completed and saved to output/mahalanobis-results-chunking.csv"
)
